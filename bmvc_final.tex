...
\begin{abstract}
	...
	localization accuracy of 0.908 at IoU $\geq$ 0.30, competitively outperforming most baselines while remaining computationally efficient 
...
of prohibited items within passenger baggage. X-ray screening remains one of the primary 
...
can only detect threat types present in their training data~\cite{baggage2022abdelfatah}. Acquiring large, balanced, and annotated datasets of real 
prohibited items is additionally expensive and less feasible.

Unsupervised Anomaly Detection offers a compelling alternative by reframing 
...
producing an elevated reconstruction error at the location of the threat. While current -
reconstruction-based anomaly detection and localization approaches have 
demonstrated promising performance, there remains room for improvement 
in accurately detecting and localizing anomalous regions. Most existing methods primarily emphasize image-level anomaly detection, whereas precise spatial localization is essential in practical baggage screening applications for effectively guiding operator attention toward suspicious regions. Furthermore, the limited number of existing reconstruction-based localization approaches still present considerable room for improvement in terms of localization accuracy, robustness, and adaptability for real-world industrial deployment.
...
\begin{enumerate}
	...
	\item \textbf{Comprehensive Evaluation on SIXray.} We benchmark against GANomaly \cite{akcay2018ganomaly}, Skip-GANomaly \cite{akcay2019skip} and unsupervised anomaly instance segmentation framework proposed by Hassan et al. \cite{hassan2021unsupervised} on the SIXray dataset \cite{miao2019sixray}. In addition, reconstruction-based anomaly localization approaches such as Patch SVDD \cite{yi2020patchsvdd}, PaDiM \cite{defard2020padim} further demonstrate the effectiveness of the localization process.
